...
- **Lane B** — recurring procedures with a fixed method → ICM stage contracts in `icm/ops/`
- Load only what the active step's context table names. Never scan the repo.
- Abbreviations resolve from the linked project first, then `icm/context/abbreviations.json`. Never invent one.
- `icm/memory/constitution.md` gates every plan.

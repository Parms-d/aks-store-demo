...
1. Read the linked project path and abbreviations path from `context/project-infra-map.md`
2. Look up the key for the resource type — e.g. `keyVaultVaults` → `kv-`
3. Missing or unreadable? Fall back to `context/abbreviations.json` and record that you did
...
| Check | Against |
|-------|---------|
| Length | Constraints table in `context/naming-standards.md` |
